...
export default function PredictionPanel({ prediction }: { prediction: OverflowPrediction | null }) {
  ...
  return <section className={`h-full rounded-2xl border border-l-4 border-slate-200 bg-white p-5 shadow-sm ${styles.accent}`}>
    ...
    {!prediction ? <p className="mt-6 text-sm text-slate-500">Select a reporting tank to calculate predictive analytics.</p> : <div className="mt-5 space-y-5">
      ...
      <div><div className="mb-1 flex justify-between text-[10px] font-semibold text-slate-500"><span>0%</span><span>65% warning</span><span>85% danger</span><span>100%</span></div><div className="relative h-3 overflow-hidden rounded-full bg-slate-100" role="progressbar" aria-label="Current tank fill" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.max(0, Math.min(100, prediction.currentLevel ?? 0))}><div className="h-full rounded-full bg-cyan-600" style={{ width: `${Math.max(0, Math.min(100, prediction.currentLevel ?? 0))}%` }} /><span className="absolute inset-y-0 left-[65%] w-px bg-amber-600" /><span className="absolute inset-y-0 left-[85%] w-px bg-red-700" /></div></div>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-xs font-semibold uppercase text-slate-600">Recommended maintenance — approval required</p><p className="mt-1 text-sm font-bold">{prediction.maintenanceRecommendation.recommendedAt ? new Date(prediction.maintenanceRecommendation.recommendedAt).toLocaleString() : "No advisory service time"}</p><p className="mt-1 text-xs text-slate-600">{prediction.maintenanceRecommendation.reason}</p><p className="mt-2 text-[11px] text-slate-500">Confidence {prediction.maintenanceRecommendation.predictionConfidence}% · safety buffer {prediction.maintenanceRecommendation.safetyBufferHours} hours. Prediction never dispatches work automatically.</p>{prediction.maintenanceRecommendation.recommendedAt && <Link href="/maintenance" className="mt-3 inline-block rounded-lg bg-cyan-700 px-3 py-2 text-xs font-bold text-white">Review for approval</Link>}</div>
