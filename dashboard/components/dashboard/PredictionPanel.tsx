...
export default function PredictionPanel({ prediction }: { prediction: OverflowPrediction | null }) {
  ...
  return <section className={`h-full rounded-2xl border border-l-4 border-slate-200 bg-white p-5 shadow-sm ${styles.accent}`}>
    <div className="flex items-start justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-slate-500">Statistical forecasting</p><h2 className="mt-1 text-lg font-semibold text-slate-900">Predictive Analytics</h2><p className="mt-1 text-sm text-slate-500">Fill-rate trend calculated from historical PostgreSQL telemetry.</p></div>{prediction && <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${styles.badge}`}>{prediction.risk} RISK</span>}</div>
    {!prediction ? <p className="mt-6 text-sm text-slate-500">Select a reporting tank to calculate predictive analytics.</p> : <div className="mt-5 space-y-5">
      <div className="rounded-xl border border-cyan-200 bg-cyan-50 p-4 text-slate-950"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-slate-500">Estimated time remaining</p><p className="mt-2 font-mono text-2xl font-bold tracking-tight">{formatCountdown(prediction.predictedOverflowAt, now)}</p><p className="mt-1 text-xs text-slate-400">{prediction.predictedOverflowAt ? `Expected ${new Date(prediction.predictedOverflowAt).toLocaleString()}` : "Level is stable or falling"}</p></div>
      ...
      <div className="rounded-xl border border-slate-200 bg-slate-50 p-3"><p className="text-xs font-semibold uppercase tracking-wide text-slate-600">Recommended maintenance</p><p className="mt-1 text-sm font-bold text-slate-900">{prediction.recommendedMaintenanceAt ? new Date(prediction.recommendedMaintenanceAt).toLocaleString() : "Routine schedule — no urgent intervention"}</p></div>
      <p className="text-[11px] text-slate-500">Deterministic statistical result based on {prediction.samples} reading{prediction.samples === 1 ? "" : "s"}. Recalculated every 30 seconds.</p>
    </div>}
  </section>;
}
