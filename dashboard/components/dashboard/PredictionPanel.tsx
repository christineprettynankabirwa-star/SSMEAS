...
export default function PredictionPanel({ prediction }: { prediction: OverflowPrediction | null }) {
  ...
  return <section className={`h-full rounded-2xl border border-l-4 border-slate-200 bg-white p-5 shadow-sm ${styles.accent}`}>
    ...
    {!prediction ? <p className="mt-6 text-sm text-slate-500">Select a reporting tank to calculate predictive analytics.</p> : <div className="mt-5 space-y-5">
      ...
      <div><div className="flex items-end justify-between"><div><p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Operational risk</p><p className="mt-1 text-3xl font-bold text-slate-950">{prediction.riskPercentage}%</p></div><p className="text-right text-xs text-slate-500">Confidence<br/><strong className="text-slate-800">{prediction.confidence}%</strong></p></div><div className="mt-3 h-2.5 overflow-hidden rounded-full bg-slate-100"><div className={`h-full rounded-full ${styles.bar}`} style={{ width: `${prediction.riskPercentage}%` }} /></div></div>
      <div className="grid grid-cols-2 gap-3 border-t border-slate-100 pt-4">
        <div><p className="text-xs font-semibold uppercase text-slate-500">Current level</p><p className="mt-1 text-xl font-bold">{prediction.currentLevel == null ? "—" : `${prediction.currentLevel.toFixed(1)}%`}</p></div>
