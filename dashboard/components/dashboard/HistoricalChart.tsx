...
export default function HistoricalChart({ tankId, tankName }: HistoricalChartProps) {
  ...
  return <section className="panel p-5"><div className="flex items-start justify-between gap-4"><div><h2 className="text-lg font-bold text-slate-900">Sensor history</h2><p className="mt-1 text-sm text-slate-500">{tankName ? `24-hour telemetry for ${tankName}.` : "Select a tank to load telemetry."}</p></div><span className="rounded-md bg-blue-50 px-2 py-1 text-xs font-semibold text-blue-700">24 hours</span></div><div className="mt-5">{tankId && loading ? <div className="h-64 animate-pulse rounded-xl bg-slate-100" /> : tankId && error ? <p className="grid h-64 place-items-center text-sm text-red-700">{error}</p> : chartData.length === 0 ? <p className="grid h-64 place-items-center text-sm text-slate-500">No historical readings are available.</p> : <div className="grid gap-5 md:grid-cols-2"><div><h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Sewage level history</h3><div className="h-56">{chart("level", "Sewage level", "#2563eb", "%")}</div></div><div><h3 className="mb-2 text-xs font-bold uppercase tracking-wide text-slate-500">Gas level history</h3><div className="h-56">{chart("gas_level", "Gas level", "#d97706", "")}</div></div></div>}</div></section>;
}
