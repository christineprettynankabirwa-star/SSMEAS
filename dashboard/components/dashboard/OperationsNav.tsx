...
export default function OperationsNav({ onSignOut }: { onSignOut: () => void }) {
  return (
    <aside className="border-b border-slate-800 bg-slate-950 text-white lg:sticky lg:top-0 lg:h-screen lg:w-64 lg:shrink-0 lg:border-b-0 lg:border-r">
      <div className="flex h-full flex-col px-4 py-5 lg:px-5 lg:py-7">
        ...
        <nav aria-label="Dashboard sections" className="mt-5 flex gap-2 overflow-x-auto pb-1 lg:mt-8 lg:flex-col lg:overflow-visible">
          {navItems.map((item, index) => <a key={item.href} href={item.href} className={`flex shrink-0 items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition hover:bg-slate-800 hover:text-white ${index === 0 ? "bg-cyan-500/10 text-cyan-300" : "text-slate-300"}`}><span aria-hidden="true" className="w-5 text-center text-base">{item.icon}</span>{item.label}</a>)}
        </nav>
