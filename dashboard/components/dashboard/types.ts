...
export interface PredictionApiResponse { tank_id: string; predicted_overflow_time: string | null; hours_remaining: number | null; predicted_minutes_to_full: number | null; average_increase_per_minute: number; risk: number; confidence: number; recommended_maintenance_date: string | null; }
...
export interface NotificationItem { id: string; alert_id: string; user_id: string; channel: "IN_APP"; status: NotificationStatus; recipient: string; subject: string; message: string; sent_at: string | null; read_at: string | null; error_message: string | null; created_at: string; tank_id: string; tank_name: string; severity: "critical" | "warning" | "info"; alert_type: string; }
