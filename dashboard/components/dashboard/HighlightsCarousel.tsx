...
export default function HighlightsCarousel({ tanks, readings, alerts, maintenance, route }: Props) {
  ...
  return <section aria-roledescription="carousel" aria-label="Operational highlights" className="group relative overflow-hidden rounded-xl border border-slate-200 bg-white text-slate-950 shadow-[0_4px_12px_rgba(0,0,0,0.05)]" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={() => setPaused(false)}><div key={slide.id} className={`ui-slide-enter min-h-52 bg-gradient-to-br ${slide.tone} px-14 py-8 sm:px-16 sm:py-10`}><p className="text-xs font-bold uppercase tracking-[.18em] text-slate-500">{slide.eyebrow}</p><div className="mt-4 max-w-2xl"><h2 className="text-2xl font-bold sm:text-3xl">{slide.title}</h2><p className="mt-2 text-lg font-semibold text-slate-800">{slide.value}</p><p className="mt-2 line-clamp-2 text-sm text-slate-500">{slide.detail}</p><Link href={slide.href} className="ui-button mt-5 inline-flex rounded-lg bg-white/80 px-4 py-2 text-sm font-bold ring-1 ring-slate-200 backdrop-blur hover:bg-white">View details →</Link></div></div>{slides.length > 1 && <><button type="button" onClick={() => move(-1)} aria-label="Previous highlight" className="ui-button absolute left-4 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-xl text-slate-700 shadow-[0_4px_12px_rgba(0,0,0,0.08)] backdrop-blur hover:bg-white">‹</button><button type="button" onClick={() => move(1)} aria-label="Next highlight" className="ui-button absolute right-4 top-1/2 grid size-9 -translate-y-1/2 place-items-center rounded-full bg-white/90 text-xl text-slate-700 shadow-[0_4px_12px_rgba(0,0,0,0.08)] backdrop-blur hover:bg-white">›</button><div className="absolute bottom-4 right-6 flex items-center gap-2">{slides.map((item, itemIndex) => <button key={item.id} type="button" aria-label={`Show highlight ${itemIndex + 1}`} aria-current={itemIndex === active} onClick={() => setIndex(itemIndex)} className={`ui-button h-2 rounded-full transition-all ${itemIndex === active ? "w-6 bg-cyan-600" : "w-2 bg-slate-300 hover:bg-slate-400"}`} />)}</div></>}</section>;
}
