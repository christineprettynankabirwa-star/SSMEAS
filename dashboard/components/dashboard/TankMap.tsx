...
function FocusTank({ tank, zoom }: { tank: Tank | undefined; zoom: number }) {
  const map = useMap();
  useEffect(() => {
    if (tank) map.setView([tank.latitude, tank.longitude], zoom, { animate: true });
  }, [map, tank, zoom]);
  return null;
}
  ...
  return <section className="panel p-5">
    ...
    <div className="mt-5 h-[380px] overflow-hidden rounded-xl border border-slate-200">
      <MapContainer center={center} zoom={mappable.length === 1 ? 13 : 10} className="h-full w-full" scrollWheelZoom={false}>
        ...
        {mappable.map((tank) => { const matchingReading = readings.find((item) => item.tank_id === tank.id); const tankReading = isLiveReading(matchingReading) ? matchingReading : null; const level = tankReading?.level; const gas = tankReading?.gas_level; const routeStop = route?.stops.find((stop) => stop.tankId === tank.id); const focused = tank.id === focusTankId; const condition = tankReading ? classifyLevel(level) : "OFFLINE"; const status=condition==="DANGER"?"Critical":condition==="WARNING"?"Warning":condition==="SAFE"?"Safe":"Offline"; const color = condition==="DANGER" ? "#dc2626" : condition==="WARNING" ? "#eab308" : condition==="SAFE" ? "#16a34a" : "#64748b"; return <CircleMarker key={tank.id} center={[tank.latitude, tank.longitude]} radius={focused ? 20 : routeStop ? 14 : 11} pathOptions={{ color: focused ? "#7f1d1d" : "#fff", weight: focused ? 7 : 3, fillColor: color, fillOpacity: 0.95, dashArray: focused ? "8 5" : undefined }}>
