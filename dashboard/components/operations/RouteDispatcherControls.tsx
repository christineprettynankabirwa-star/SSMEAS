...
export default function RouteDispatcherControls({ route, busy, onRecalculate }: { route: OptimizedRoute; busy: boolean; onRecalculate: (request: RouteOptimizationRequest) => void }) {
  ...
  const [planningHorizonHours, setPlanningHorizonHours] = useState(24);
  ...
  const submit = () => {
    ...
    onRecalculate({ truckId, driverId: driverId || undefined, truckCapacityLiters: truck.capacity, planningHorizonHours, excludedTankIds: excluded, preferredOrder: order, lockedTankIds: locked });
  };
  return <section className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
    <div className="flex flex-col gap-4 lg:flex-row lg:items-end"><label className="flex-1 text-xs font-bold text-slate-600">Collection Truck<select value={truckId} onChange={(event) => setTruckId(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm">{trucks.map((truck) => <option value={truck.id} key={truck.id}>{truck.label}</option>)}</select></label><label className="flex-1 text-xs font-bold text-slate-600">Driver<select value={driverId} onChange={(event) => setDriverId(event.target.value)} className="mt-1.5 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm"><option value="">Unassigned</option>{route.availableDrivers.map((driver) => <option key={driver.id} value={driver.id}>{driver.name}</option>)}</select></label><label className="flex-1 text-xs font-bold text-slate-600">Planning Horizon<input type="number" min={1} max={168} value={planningHorizonHours} onChange={(event) => setPlanningHorizonHours(Math.max(1, Math.min(168, Number(event.target.value) || 24)))} className="mt-1.5 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm" /><span className="mt-1 block font-normal text-slate-500">hours to projected 85%</span></label><button type="button" onClick={submit} disabled={busy} className="h-10 rounded-lg bg-cyan-700 px-5 text-sm font-bold text-white hover:bg-cyan-800 disabled:opacity-50">{busy ? "Recalculating..." : "Recalculate Route"}</button></div>
