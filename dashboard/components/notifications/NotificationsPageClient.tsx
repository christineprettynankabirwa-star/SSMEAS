...
import {
  ...
  testNotificationSms, updateNotificationPreferences,
} from "@/services/api";
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        {loading ? <div className="h-60 animate-pulse rounded-xl bg-slate-100" /> : items.length ? <div className="space-y-3">{items.map((item) =>
          <div key={item.id} className="relative">
            <button type="button" onClick={() => void read(item)} className={`block w-full rounded-xl border p-4 pr-10 text-left ${item.read_at ? "border-slate-200 bg-slate-50 opacity-70" : "border-cyan-200 bg-white shadow-sm"}`}>
            ...
            <button type="button" onClick={(event) => { event.stopPropagation(); void clear(item.id); }}
              className="absolute right-2 top-2 grid size-6 place-items-center rounded-lg text-xs text-slate-400 hover:bg-slate-200 hover:text-slate-700"
              aria-label="Dismiss notification">×</button>
          </div>)}</div> : <p className="p-16 text-center text-sm text-slate-500">No notifications have been recorded.</p>}
      </section>
